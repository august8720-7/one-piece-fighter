...
if (!['candidate-0912', 'candidate-0913'].includes(release)) throw new Error('Unknown local candidate release.');
const full = process.argv.includes('--full');
if (full && release !== 'candidate-0913') throw new Error('--full requires --release candidate-0913.');
const root = resolve(dirname(fileURLToPath(import.meta.url)), '..', '.local-releases', release);
const portIndex = process.argv.indexOf('--port');
const port = portIndex < 0 ? release === 'candidate-0913' ? 4177 : 4176 : Number(process.argv[portIndex + 1]);
